module Services
  module Sales
    class OrderInvoiceService
      ...
      def create_invoices
        invoice_data_matrix = determine_order_invoices_matrix
        response_array = []
        invoice_data_matrix.each do |invoice_data|
          invoice_line_ids = []
          ...
          invoice_data_hash = {
            ...
            "customer_ruc": @order.wants_factura ? @order.customer.customer.factura_ruc : @order.customer.doc_id,
            "customer_comprobante_tipo_catalog_6": @order.wants_factura ? "6" : Customer.doc_types[@order.customer.doc_type],
            "customer_name": @order.wants_factura ? @order.customer.customer.factura_razon_social : @order.customer.user.full_name,
          ...
          response_array << response.parsed_response
      ...
      def determine_order_invoices_matrix
        # iterate over each order payments
        invoice_series_array = []
        @order.payments.each do |payment|
          invoice_series_ids = InvoiceSeriesMapping.where(location_id: @order.location_id, payment_method_id: payment.payment_method_id).pluck(:invoice_series_id)
          ...
          if invoice_series.count == 1
            ...
            invoice_series = invoice_series.find_by(is_default: true)
          end
          ...
          invoice_series_array << invoice_data
        end
